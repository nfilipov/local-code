...
my $pdfPrefix = "pdfOutput/2014_0418/";
my $txtPrefix = "txtOutput/2014_0418/";
...
my @sampleIndex  = ("","1","2","3","4","5","6","7","8"); #Input data sample. 1: pp@7TeV-d0 data ; 2: pp@7TeV-d3 data; 3: PbPb@276 regit 4: pp reco TT 5: pp reco GG 6: zhen tree, GG 7: pp@2.76TeV data
my $samstart = 3;
my $samend   = 3;
my @choseSamples = ("","pp7tev-d0", "pp7tev-d3","pbpbRegit_cm","pbpbPPrecoTT", "pbpbPPrecoGG","pbpbZhen2011","pp2p76tev","ppMC1S");
# ------------------------------------------------------
my $doNarrowMass = 0; 
my $choseFitParams = 0; #what you want 0: (1s, 2s, 3s) 1: (1s, 2s/1s; 3s/1s); 2: (1S, (2s+3s)/1s); 3:(1S, 2S/1S, 3S/1S, 3S/2S)
if($samstart == 8){$choseFitParams=0;}
...
# new rap bins
my @rapBinMin = ("0.","0.4" ,"0.8","1.2","1.6","0" ,"1.2.","0.");
my @rapBinMax = ("0.4","0.8","1.2","1.6","2.4","1.2","2.4","2.4");
my $whatBin=0;
my $rapStart =0;
my $rapEnd = 7;
my $ptStart = 10;
...
my @upsPtBinMin = ("0","2.5","5","8" ,"12","20" ,"0"  ,"5","12","20","0");	 
my @upsPtBinMax = ("2.5","5" , "8","12","20","50","5","12","20","50","100");
my $dontDoRapNow =0;
my $dontDoPtNow =1;
		...
		for (my $iPtMu=$ptMuStart2; $iPtMu<=$ptMuEnd2; $iPtMu++)
		{
			...
			for (my $ifsr=$fsrstart; $ifsr<=$fsrend; $ifsr++)
			{
				...
				for ( my $ibkg=$modelstart; $ibkg<=$modelend; $ibkg++)
				{
				    ...
				    system("root -l -b -q	'fitUpsilonYields.C($choseSample,$choseFitParams,$bkgType,$myfsr,$mysigma1,$centMin,$centMax,$muonEtaMin,$muonEtaMax,$dimuYMin,$dimuYMax,$muonPtCut1,$muonPtCut2,$plotBkg,$doTrkRot,$doConstrainFit,$myref,$paramsOnFigure,\"$sample\",\"$outFigDir\",\"$outDatDir\",\"$prefix\",$doNarrowMass,$vProb)'
			"); 
				}	#bkg 
			    } # ref
			}#fsr
...
for ( $isam=$samstart; $isam<=$samend; $isam++)
{
 ...
 #   if ($isam==1) {$doNarrowMass=1;}
	...
	for (my $iPtMu1=$ptMuStart1; $iPtMu1<=$ptMuEnd1; $iPtMu1++)
	{
	    my $muonPtCut1      = $chooseptMu[$iPtMu1]; #single muon pT cut
	    for (my $iPtMu=$ptMuStart2; $iPtMu<=$ptMuEnd2; $iPtMu++)
	    {
		my $muonPtCut2      = $chooseptMu[$iPtMu]; #single muon pT cut
		$ptMuPrefix = $outFigPrefix_ptMu[$iPtMu1];	
		# these two lines are repeated because $outFigDir and $outDatDir go in the function's arguments
		$outFigDir = $outFigDirProb.$ptMuPrefix."/";
		$outDatDir = $outDatDirProb.$ptMuPrefix."/";
		...
		for (my $isig=$sigstart; $isig<=$sigend; $isig++)
		{
			...
			for (my $iref=$refstart; $iref<=$refend; $iref++)
			{
				...
				for ( my $ibkg=$modelstart; $ibkg<=$modelend; $ibkg++)
				{
				    ...
				    print    " $upsPtCutMin < pT_Ups < $upsPtCutMax \n";
				    print     " $dimuYMin < y_Ups < $dimuYMax \n";
				    system("root -l -b -q	'fitUpsilonYields_Yvariant.C($whatBin,$choseSample,$choseFitParams,$bkgType,$myfsr,$mysigma1,$centMin,$centMax,$muonEtaMin,$muonEtaMax,$dimuYMin,$dimuYMax,$muonPtCut1,$muonPtCut2,$plotBkg,$doTrkRot,$doConstrainFit,$myref,$paramsOnFigure,\"$sample\",\"$outFigDir\",\"$outDatDir\",\"$prefix\",$doNarrowMass,$vProb)'
			"); 
				} #bkg 
			    } # rapidity bins
				...
				for ( my $ibkg=$modelstart; $ibkg<=$modelend; $ibkg++)
				{
				    ...
				    print "$upsPtCutMin > pT_Ups > $upsPtCutMax";
				    print "$dimuYMin > y_Ups > $dimuYMax";
				    system("root -l -b -q	'fitUpsilonYields_variant.C($whatBin,$choseSample,$choseFitParams,$bkgType,$myfsr,$mysigma1,$centMin,$centMax,$muonEtaMin,$muonEtaMax,$upsPtCutMin,$upsPtCutMax,$muonPtCut1,$muonPtCut2,$plotBkg,$doTrkRot,$doConstrainFit,$myref,$paramsOnFigure,\"$sample\",\"$outFigDir\",\"$outDatDir\",\"$prefix\",$doNarrowMass,$vProb)'
			"); 
				} #bkg 
			    } # pT bins
			} # ref
